...
def read_messages(dbfile):
    ...
    c.execute("SELECT _id, thread_id, address, person, date,  read, priority,  type, callback_number, reply_path_present, subject, body, failure_cause,  error_code, stack_type, seen, sort_index FROM mmssms ORDER BY date DESC")
    while True:
        row = c.fetchone()
        if row is None: break

        rec_id, thread_id, address, person, date,  read, priority,  type, callback_number, reply_path_present, subject, body,  failure_cause,  error_code, stack_type, seen, sort_index = row
